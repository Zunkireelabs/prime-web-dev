...
export default function DealersGrid({
  ...
  return (
    <section id="dealers-grid" className="bg-surface" style={{ padding: "clamp(100px, 12vw, 180px) 0" }}>
      <div className="container">
        ...
        <FadeIn delay={0.25}>
          <div className="bg-surface-alt border border-ink/6" style={{ padding: "clamp(24px, 4vw, 48px)", marginBottom: "56px" }}>
            {/* Search */}
                  ...
                  <button
                    ...
                    className={`cursor-pointer px-5 py-3 text-[0.7rem] font-medium tracking-[0.15em] uppercase transition-all duration-300 border ${
                      isActive
                        ...
                        : "bg-white border-ink/10 text-ink-light hover:border-accent/40 hover:text-ink"
                    }`}
                  >
                    {province}
                    <span className={isActive ? "text-white/70 ml-2" : "text-ink-muted ml-2"}>
                      ({count})
                    </span>
                  </button>
                );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" style={{ gap: "24px" }}>
            {visible.map((dealer, i) => (
              <FadeIn
                ...
                distance={20}
              >
                <div className="group relative border border-ink/6 hover:border-accent/30 bg-white transition-all duration-300 h-full hover:shadow-[0_4px_24px_rgba(0,0,0,0.06)]" style={{ padding: "28px" }}>
                  {/* Accent top line */}
                  <div className="absolute top-0 left-0 right-0 h-[2px] bg-accent origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-300" />

                  {/* Dealer Name */}
                  <h3 className="text-[0.9rem] font-normal text-ink group-hover:text-accent transition-colors duration-300 leading-snug" style={{ marginBottom: "16px" }}>
                    {dealer.name}
                  </h3>

                  {/* Details */}
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                    <p className="flex items-start gap-4 text-[0.8rem] text-ink-light leading-relaxed">
                      <MapPin
                        size={14}
                    ...
                    {dealer.phone && (
                      <p className="flex items-center gap-4 text-[0.8rem] text-ink-light">
                        <Phone
                          size={14}
                    ...
                    {dealer.contactPerson && (
                      <p className="flex items-center gap-4 text-[0.8rem] text-ink-muted">
                        <User
                          size={14}
        ...
        {hasMore && (
          <div style={{ textAlign: "center", marginTop: "64px" }}>
            <button
              ...
              className="cursor-pointer px-10 py-4 text-[0.65rem] font-medium tracking-[0.2em] uppercase text-ink border border-ink/15 hover:border-accent hover:text-accent transition-all duration-300"
            >
              Load More Dealers
            </button>
          </div>
